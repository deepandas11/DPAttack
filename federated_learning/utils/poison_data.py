from .label_replacement import apply_class_label_replacement, apply_random_noise
from .client_utils import log_client_data_statistics

def poison_data(logger, distributed_dataset, num_workers, poisoned_worker_ids, replacement_method, attack='label_replace'):
    """
    Poison worker data
    ...
    for worker_idx in range(num_workers):
        if worker_idx in poisoned_worker_ids:
            if attack == 'label_replace':
                ...
                poisoned_dataset.append(apply_random_noise(distributed_dataset[worker_idx][0], distributed_dataset[worker_idx][1], replacement_method))
        else:
