...
def run_exp(replacement_method, num_poisoned_workers, KWARGS, client_selection_strategy, idx, name, attack='label_replace'):
    log_files, results_files, models_folders, worker_selections_files = generate_experiment_ids(idx, 1, name)
    print("\n\n Models Folders : ", models_folders)
    ...
    print("\n\n\nPoisoned Workers are : ", poisoned_workers, len(poisoned_workers))
    distributed_train_dataset = poison_data(logger, distributed_train_dataset, args.get_num_workers(), poisoned_workers, replacement_method, attack)
